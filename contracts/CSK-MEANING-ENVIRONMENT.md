...
operational authority. For the checked CSK Profile subset, the Meaning
...
reads exactly one CSK Profile input datum from a file path and binds it as the
...
The evaluator implements fresh intrinsic semantics for the closed CSK Profile
intrinsic set:
...
use `eqv?`. `assq` and `memq` are not CSK Profile intrinsics.

`map`, `filter`, `reduce`, `fold-left`, and `fold-right` accept a callable value
and a proper list. Their callbacks are applied through the same graph evaluator
and step accounting as ordinary calls. `reduce` and `fold-left` call `(f acc
elem)` left-to-right. `fold-right` calls `(f elem acc)` right-to-left.
...
Later slices add verify/replay UX and external oracle material. Topaz is
