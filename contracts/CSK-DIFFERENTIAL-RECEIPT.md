...
> interpreter and the Meaning Environment, optionally over one pinned CSK Profile
> host input datum. It is not a semantic equivalence proof and not an independent
> witness.

## 1. Purpose

The differential receipt records one source input, one optional CSK Profile host
...
authority. For the lowered CSK Profile subset, a disagreement is an
...
- CSK Profile input hash domain: `csk/profile-input-hash/v0`.
...
`list->vector` intrinsics, and the v1.2.14 CSK Profile builtin set.
...
Meaning Environment report, joined as each entry followed by LF.
