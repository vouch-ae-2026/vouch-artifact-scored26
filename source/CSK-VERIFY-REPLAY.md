...
From W3.4, `lispex verify` applies read-side canonical acceptance to native
differential receipts before semantic receipt checks. The checker parses the
submitted JSON only to obtain a candidate value, rewrites that value with the
current native artifact writer profile, and byte-compares the rewrite with the
submitted bytes. If they differ, verification exits `1` with
`non-canonical-artifact-json`. Non-canonical receipts are not repaired and then
accepted. Duplicate-key submissions are reported through this
`non-canonical-artifact-json` path; v0 does not claim a dedicated
...
the same native receipt canonical reader and `lispex verify` core checks. A
non-canonical baseline receipt therefore fails the replay precondition before
receipt semantic checks.
...
as the corpus baseline. Candidate receipts must pass the same native receipt
canonical reader and verify core.
