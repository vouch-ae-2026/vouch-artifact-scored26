...
## 13. Example Conversion-Engine Alignment

The external converter remains a separate product and runs on the example conversion engine. The
...
```text
The example conversion engine performs the conversion. Lispex Vouch supplies the public
...
```text
Lispex runs the external converter.
...
external converter's repository has already emitted this exact artifact.
