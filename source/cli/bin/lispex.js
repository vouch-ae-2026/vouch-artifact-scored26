...
playground (https://artifact.invalid/lispex) runs — identical core, identical results.
...
const artifactReadFailures = new WeakMap();
...
function sameDivergence(left, right) {
  if (left === null || right === null) return left === right;
  return (
    left?.index === right?.index &&
    left?.reference === right?.reference &&
    left?.meaning_env === right?.meaning_env
  );
}
...
function checkNativeClosedWorld(receipt, failures) {
  expectAllowedKeys(
    receipt.boundary,
    ['attests', 'excludes'],
    failures,
  ...
  expectAllowedKeys(
    receipt.engine,
    ['canonical_format', 'commit', 'name', 'version'],
    failures,
    'engine'
  );
  ...
  expectHashObjectClosed(receipt.source?.hash, failures, 'source.hash');
  expectAllowedKeys(
    receipt.input,
    ['byte_len', 'datum', 'hash', 'name', 'path', 'status'],
    failures,
    'input'
  );
  if (isObject(receipt.input?.hash)) {
    expectHashObjectClosed(receipt.input.hash, failures, 'input.hash');
  }
  expectAllowedKeys(
    receipt.canonical,
    ['byte_len', 'hash', 'status'],
    failures,
    'canonical'
  );
  if (isObject(receipt.canonical?.hash)) {
    expectHashObjectClosed(receipt.canonical.hash, failures, 'canonical.hash');
  }
  expectAllowedKeys(
    receipt.graph,
    ['byte_len', 'col', 'hash', 'kind', 'line', 'message', 'status'],
    failures,
    'graph'
  );
  if (isObject(receipt.graph?.hash)) {
    expectHashObjectClosed(receipt.graph.hash, failures, 'graph.hash');
  }
  expectAllowedKeys(
    receipt.reference,
    ['hash', 'status', 'transcript', 'transcript_byte_len'],
    failures,
    'reference'
  );
  if (isObject(receipt.reference?.hash)) {
    expectHashObjectClosed(receipt.reference.hash, failures, 'reference.hash');
  }
  expectAllowedKeys(
    receipt.meaning_env,
    ['fault', 'hash', 'status', 'steps', 'transcript', 'transcript_byte_len'],
    failures,
    'meaning_env'
  );
  if (isObject(receipt.meaning_env?.hash)) {
  ...
  expectAllowedKeys(
    receipt.meaning_env?.steps,
    ['limit', 'used'],
    failures,
    'meaning_env.steps'
  );
  if (isObject(receipt.meaning_env?.fault)) {
    expectAllowedKeys(
      receipt.meaning_env.fault,
      ['anchor', 'kind', 'message', 'node'],
      failures,
      'meaning_env.fault'
    );
    expectAllowedKeys(
      receipt.meaning_env.fault.anchor,
      ['col', 'line'],
      failures,
      'meaning_env.fault.anchor'
    );
  }
  expectAllowedKeys(
    receipt.comparison,
  ...
  if (isObject(receipt.comparison?.first_divergence)) {
    expectAllowedKeys(
      receipt.comparison.first_divergence,
      ['index', 'meaning_env', 'reference'],
      failures,
      'comparison.first_divergence'
    );
  }
  if (Array.isArray(receipt.comparison?.blockers)) {
    receipt.comparison.blockers.forEach((blocker, index) => {
      expectAllowedKeys(
        blocker,
        ['fault_class', 'reason'],
        failures,
        `comparison.blockers[${index}]`
      );
    });
  }
  if (Array.isArray(receipt.diagnostics)) {
    receipt.diagnostics.forEach((diagnostic, index) => {
      expectAllowedKeys(
        diagnostic,
        ['code', 'col', 'file', 'line', 'message', 'severity'],
        failures,
        `diagnostics[${index}]`
      );
    });
...
function verifyReceipt(receipt, options = {}) {
  ...
  checkNativeClosedWorld(receipt, failures);
...
function verifyCommand(args) {
  ...
  const artifact = readJsonArtifactFile(receiptPath, 'lispex verify', 'native');
  ...
  const core =
    artifact.canonicalFailures.length > 0
      ...
      : verifyReceipt(receipt, { sourcePath });
...
function readReceiptDir(dir, command) {
  ...
  for (const entry of listJsonFiles(dir, command)) {
    ...
    if (isObject(artifact.value) && artifact.canonicalFailures.length > 0) {
      artifactReadFailures.set(artifact.value, artifact.canonicalFailures);
    }
    receipts.set(stemOf(entry), artifact.value);
  }
  return receipts;
}
...
function verifyPrecondition(receipts, label) {
  ...
  for (const [stem, receipt] of receipts) {
    const readFailures = isObject(receipt)
      ? artifactReadFailures.get(receipt)
      : null;
    if (readFailures?.length > 0) {
      failures.push(`${label}/${stem}: ${readFailures.join(', ')}`);
      continue;
    }
...
module.exports = {
  BRIDGE_TAG,
  BRIDGE_TOP_LEVEL_FIELDS,
  DIFF_RECEIPT_TOP_LEVEL_FIELDS,
  DIFF_TAG,
};
