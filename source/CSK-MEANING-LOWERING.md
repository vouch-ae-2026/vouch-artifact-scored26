...
- `Values`, lowered through the closed `values` intrinsic call shape
...
inversion, external backend witnessing, target-code generation, full Core Semantic Kernel coverage, or
...
| `Var(Ident::User)` for a checked profile builtin | `ref` with `{ "space": "intrinsic", "name": ... }` |
...
| `Values(exprs)` | `call` whose operator is the closed `values` intrinsic and whose args are lowered `exprs` |
...
The checked profile reserves its builtin names and the host-input name `input`. A
...
`eq?` is not part of the checked profile comparison surface.
...
`Values` lowers without adding a graph node kind. The lowerer emits a `call`
node whose operator is an intrinsic `values` ref. This admits normalizer-generated
zero-values branches for derived forms such as `when`, `unless`, and `cond`
without claiming full multiple-values coverage beyond the closed intrinsic
surface implemented by the Meaning Environment.
...
`profile-escape` for reserved checked profile binders.
