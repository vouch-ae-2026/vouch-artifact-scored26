...
Version 8.6 closes DEV-002 and the condition-ledger gate exposed by the M7 external audit. It introduces `csk.checked-profile/v1` and `csk.checked-input/v1`, adds the total `exact-integer?` predicate to the checked primitive set, makes tagged rationals disjoint from JSON integers by requiring a denominator greater than one, requires workload rules to check all five positional values before numeric operations, and requires every one of the 213 condition rows to be uniquely present, fixture-backed, and `built` before any release gate can pass. Version 8.5.1 remains an immutable historical contract for the preceding 211/213 release and is not retroactively reinterpreted.
...
The Meaning Environment path and the reference path MUST be compared over the higher-order functional subset defined by `csk.checked-profile/v1`. Lambdas are first-class values. The subset MUST NOT be described as first-order.
...
```text
+ - * / = < <= > >= cons car cdr null? pair? list exact-integer?
decision-approve
decision-deny
decision-review
...
A tagged rational denominator MUST be greater than one. Numerator and denominator MUST be coprime, and the numerator MUST be nonzero. Every mathematical integer, including zero, MUST use the JSON integer production. This makes the closed host-value alternatives disjoint and preserves the application-visible distinction required by P-11 after P-3 mapping.
...
Records are outside checked-input version 1. An object is valid only when it has exactly one `$rat`, `$real`, or `$sym` tag and the exact nested schema above. Arbitrary JSON objects and maps are forbidden.
...
### P-4 Malformed and invalid input (amended v8.6)
...
The checked workload program MUST test `exact-integer?` on every positional element before applying `=`, `<`, `<=`, `>`, or `>=` to that element. Over the P-2 host-value grammar, the predicate is total and returns true exactly for an exact Lispex integer; it returns false for booleans, strings, lists, rationals, binary64 reals, and symbols. The existing decision-operand and observable-procedure profile boundaries remain unchanged. Both evaluators MUST implement this primitive independently and emit identical observable traces for it.
...
### P-11 Application input schema (amended v8.6)
...
The application schema is identified as `csk.checked-input/v1`. Its arity, position table, category-code table, and numeric ranges form a closed workload schema. A conforming workload implementation MUST reject unlisted category codes at the application layer by returning `decision-invalid-input`.
...
The fixture lane MUST exercise every noninteger P-2 host-value alternative at every one of the five positions under both frozen rule versions. It MUST also exercise short and long arities and every category/range rejection class. A language fault, profile escape, pipeline failure, or not-comparable result in this lane is a conformance failure.
...
Checked profile version 1 has no covered output primitive. A version 1 checked-profile transcript MUST contain no output event. The output event variant is reserved and cannot be emitted by `csk.checked-profile/v1`.
  ...
  "keys": [
    {
      ...
      "allowed_engine_sha256": ["sha256:<64 lowercase hex digits>"]
    }
  ]
}
```
...
The profile identifier MUST pass C-POLICY-06 syntax. The only profile supported for
`csk.differential-receipt/v0` issuance is:
...
The workload application value MUST be a positional `csk.checked-input/v1` list with this exact shape:
...
canonical_input_bytes
  ...
  csk.checked-input/v1 file, including its single final LF,
...
canonical_input_bytes
  the exact complete bytes of the case's canonical csk.checked-input/v1
...
### C-FINAL-02 No partial conformance (amended v8.6)

The release final gate is the phase-3 publication-check of C-REP-08, whose terminal report S carries the release verdict. A release MUST fail that final gate when any generated result, digest, error class, visible string, toolchain pin, command outcome, schema rule, signature, deterministic derivation, manifest expectation, condition-ledger expectation, or paper value violates this contract. Phase 1 MUST verify that the active contract contains exactly 213 unique condition identifiers; that the condition map is their exact ordered projection; that every row has `scope = built`, `implementation_status = built`, and at least one known fixture identifier; and that both the active v8.6.0 and historical v8.5.1 contract digests match their committed SHA records. This check MUST be a built fixture in the exact fixture report consumed by Q. A failed or omitted condition-ledger fixture prevents Q from passing, and therefore prevents a later S from passing. Phase 3 MUST itself re-evaluate every deterministic qd and rd derivation named by the C-REP-08 replay block from its frozen buffers, so every such derivation violation fails the terminal gate even when R has a valid release-key signature. Checks that phase 1 can make fail phase 1; checks that depend on the signed observation R, the publication record P, or the paper fail phase 3. Version 8.6.0 has no design-target condition or fixture rows, and none may be reported as conformant by exclusion.
