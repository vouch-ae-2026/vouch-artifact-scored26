...
The checked profile is the Lispex source subset that may participate in Lispex
...
`any?` and `all?` are checked profile extensions, not R7RS claims. Both take exactly
...
The vNext transcription and welfare evidence keeps this profile surface
unchanged. The transcription-fidelity ledger maps five rules from JSON Logic,
Cedar, and OpenFisca Core into the checked profile under declared input/output
maps and records 676/676 declared-map agreement. The profile-growth batch
proposal recommends zero L2 changes, zero L3 changes, and no profile version
bump over that evidence. This is evidence that the current profile can host the
current rule corpus; it is not a semantic-equivalence or policy-correctness
claim about the upstream systems.
...
- external backend independence or external witnessing
...
The welfare replay corpus and its evaluation record are outside this Vouch
source projection; the executable profile decision above is preserved here.
